...
const EncuentroEgresados: React.FC = () => {
  const [items, setItems] = useState<EgresadosItem[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  // Modal
  const [open, setOpen] = useState<boolean>(false);
  const [active, setActive] = useState<EgresadosItem | null>(null);

  useEffect(() => {
    let mounted = true;
    setLoading(true);

    fetch(`${BACKEND_URL}/api/egresados-encuentros`)
      .then((res) => {
      ...
      .then((data: any[]) => {
        ...
        if (Array.isArray(data)) {
            const mappedItems: EgresadosItem[] = data.map((item) => ({
                id: item.id,
                title: item.titulo,
                description: item.descripcion,
                fileUrl: `${BACKEND_URL}${item.url}`,
                type: item.tipo,
                date: item.fecha_subida ? new Date(item.fecha_subida).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' }) : ''
            }));
  ...
  const openPreview = (item: EgresadosItem) => {
    setActive(item);
    setOpen(true);
    ...
    <div className="max-w-6xl mx-auto p-4">
      <header className="mb-8 text-center">
        ...
        <p className="text-gray-600 mt-2 max-w-3xl mx-auto">
          Espacio dedicado a nuestros egresados para compartir experiencias, logros y mantener el vínculo con su alma mater.
        </p>
      </header>
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  {it.date && (
                    <p className="text-xs text-gray-400 mt-2">Publicado: {it.date}</p>
                  )}
                </div>
