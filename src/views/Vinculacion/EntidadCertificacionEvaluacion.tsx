...
const EntidadCertificacionEvaluacion: React.FC = () => {
  ...
  const [active, setActive] = useState<EntidadItem | null>(null);

  useEffect(() => {
    let mounted = true;
    setLoading(true);

    fetch(`${BACKEND_URL}/api/entidad-certificacion-evaluacion`)
      .then((res) => {
      ...
      .then((data: any[]) => {
        ...
        if (Array.isArray(data)) {
            const mappedItems: EntidadItem[] = data.map((item) => ({
                id: item.id,
                title: item.titulo,
  ...
  const openPreview = (item: EntidadItem) => {
    setActive(item);
    setOpen(true);
    ...
    <div className="max-w-6xl mx-auto p-4">
      <header className="mb-8 text-center">
        ...
        <p className="text-gray-600 mt-2 max-w-3xl mx-auto">
          Centro evaluador acreditado para la certificación de competencias laborales.
        </p>
      </header>
