...
import {
  ...
  Users, X, FileCheck, Hospital, IdCard, Scroll, BookOpen
} from 'lucide-react'
...
// Servicios estáticos - Combinando iconos de master con rutas de pruebas-becas
          ...
          <div className="grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-items-center">
            {servicios.map((servicio, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="w-full max-w-[220px] h-[190px]"
                >
                  <Link 
                    to={servicio.href || "#"}
                    className="cursor-pointer transition-all duration-200 hover:scale-105 hover:drop-shadow-xl w-full block"
                  >
                    <ServicioCard {...servicio} />
                  </Link>
                </div>
