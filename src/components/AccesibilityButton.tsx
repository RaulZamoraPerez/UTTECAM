...
// React types not required in modern TSX with automatic JSX runtime
...
const ToggleOption = ({ icon, label, subtitle, active = false, onClick = () => { }, delay = 0.1, activeColor = '#16a34a' }: any) => {
    return (
        <motion.div
        ...
        >
            <div className={`p-2 rounded-md bg-gray-50 border border-slate-100`}>{icon}</div>
            ...
            <div className="w-16 text-right">
                <button
                    onClick={onClick}
                    className={`px-3 py-1 rounded-lg font-medium text-sm ${active ? 'bg-opacity-100' : 'bg-gray-100'} text-white`}
                    style={active ? { backgroundColor: activeColor } : { backgroundColor: '#d1d5db', color: '#111827' }}
                >
                    {active ? 'ON' : 'OFF'}
                </button>
            </div>
