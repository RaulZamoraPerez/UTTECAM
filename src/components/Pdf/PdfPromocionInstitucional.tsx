...
import "react-medium-image-zoom/dist/styles.css";
 ...
 const PdfPromocionInstitucional = () => {
  ...
  const Url = `/ExtensionUniversitaria/DifusionyDivulgacion/PromocionInstitucional/${title?.trim()}`;
        ...
        isMobile ? (
          <div className="flex-1 flex flex-col items-center justify-center  p-6">
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
              ...
              <h2 className="text-xl font-semibold text-gray-700 mb-2">Visor no compatible en móviles</h2>
          ...
          <>
            {isLoading && (
              <div className="flex flex-col items-center">
                <Spinner text="Cargando documento... Puede tardar varios minutos si el archivo es pesado." />
                <p className="text-sm text-gray-500 mt-2">
                  Si el PDF no carga después de un tiempo, prueba descargarlo directamente.
                </p>
                <a
                  href={Url}
                  download
                  className="inline-block bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-6 rounded-full transition duration-200 mt-4"
                >
                  Descargar PDF
                </a>
              </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full text-center">
            <Zoom>
              <img
                src={Url}
                alt={formatearTitulo(title!)}
                className="mx-auto rounded-lg max-h-[70vh] object-contain"
                onLoad={() => setIsLoading(false)}
                style={{ display: isLoading ? "none" : "block" }}
              />
            </Zoom>
            {isLoading && <Spinner text="Cargando imagen..." />}
            <a
              href={Url}
              download
              className="inline-block bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-6 rounded-full transition duration-200 mt-4"
            >
              Descargar imagen
            </a>
          </div>
